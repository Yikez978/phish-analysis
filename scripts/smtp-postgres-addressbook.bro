...
@ifdef ( Log::WRITER_POSTGRESQL )
	local filter: Log::Filter = [$name="postgres_addressbook_rec", $path="addressbook", $writer=Log::WRITER_POSTGRESQL, $config=table(["conninfo"]="host=pg.lbl.gov dbname=cluster4.lbl.gov user=bro password=brodatabase")];
